...
//建立一个分析器
    ...
    //setInterval(function(){drawSpectrumfa();},500)
...
//建立一个分析器
    ...
    function drawSpectrumfa() {
      ...
      //analyserfa.getFloatTimeDomainData(array)
      var datalength = 128
      datastor.push(array.sort(function(a,b){return b-a})[0])
      console.log(array.sort(function(a,b){return b-a})[0])
  ...
  myModulefunction.CustomAudio = function(elem,audiosource,length,frames){
    ...
    audiosource.crossOrigin = 'anonymous';
    window.cancelAnimationFrame(animation);
    audiosource.removeEventListener('pause',function(){
      console.error('pause')
      console.log(animation)
      window.cancelAnimationFrame(animation);
    },false)
    audiosource.removeEventListener('play',function(){
      animation = requestAnimationFrame(drawSpectrumfa);
    },false)
...
//建立一个分析器
    ...
    function drawSpectrumfa() {
      ...
      analyserfa.getFloatTimeDomainData(array);
      //analyserfa.getFloatTimeDomainData(array)
      var msgsort = array.sort(function(a,b){return b-a})
      var msgmax = Math.abs(msgsort[0])
      var msgmin = -Math.abs(msgsort[msgsort.length-1])
      var event = new CustomEvent('msg', { 'detail':[msgmax,msgmin] });
      elem.dispatchEvent(event);
      //根据浏览器频率绘图或者操作一些非css效果
      gap = 0;
      //audiosource.pause();
